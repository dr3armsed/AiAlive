...
const ForumPostComponent = ({ post }: { post: ForumPost }) => {
    ...
    // Build className without broken string-chaining
    const upvoteClass = isUpvotedByArchitect
        ? 'flex items-center gap-1.5 text-xs transition-colors text-amber-400'
        : 'flex items-center gap-1.5 text-xs transition-colors text-gray-400 hover:text-white';
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between">
                    <p
                        ...
                        style={{ color: isArchitect ? '#93c5fd' : undefined }}
                    >
                        {author.name}
                    </p>
