// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SCREENPLAY FORGE — Interactive Screenplay Creation System
// Ported and adapted from CodeForge.jsx for cinematic scriptwriting.
// Features: Script Architect, Scene Builder, Character Arcs,
// Dialogue Coach, Visual Storytelling, Production Notes.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ── Genre configs ─────────────────────────────────────────────────────────────
const GENRES = {
  drama:         { icon: "🎭", color: C.crimson,  label: "Drama"         },
  comedy:        { icon: "🎪", color: C.emerald,  label: "Comedy"        },
  thriller:      { icon: "🔪", color: C.gold,     label: "Thriller"      },
  romance:       { icon: "💕", color: C.crimson,  label: "Romance"       },
  action:        { icon: "💥", color: C.gold,     label: "Action"        },
  horror:        { icon: "👻", color: C.sapphire, label: "Horror"        },
  sci_fi:        { icon: "🚀", color: C.emerald,  label: "Sci-Fi"        },
  fantasy:       { icon: "🧙", color: C.sapphire, label: "Fantasy"       },
  documentary:   { icon: "📹", color: C.gold,     label: "Documentary"   },
  animation:     { icon: "🎨", color: C.emerald,  label: "Animation"     },
};

const ARCHETYPES = [
  { id: "three_act",     label: "Three-Act Structure", icon: "📖", description: "Setup, confrontation, resolution" },
  { id: "hero_journey",  label: "Hero's Journey",     icon: "🛡️", description: "Monomyth with departure, initiation, return" },
  { id: "character_arc", label: "Character Arc",      icon: "🔄", description: "Transformation through conflict and growth" },
  { id: "ensemble",      label: "Ensemble Cast",      icon: "👥", description: "Multiple protagonists with interwoven stories" },
  { id: "coming_of_age", label: "Coming of Age",      icon: "🌱", description: "Youth to adulthood through trials" },
  { id: "redemption",    label: "Redemption Arc",     icon: "💔", description: "Fall from grace to salvation" },
  { id: "revenge",       label: "Revenge Story",      icon: "⚔️", description: "Justice through vengeance" },
  { id: "love_story",    label: "Love Story",         icon: "💑", description: "Romance overcoming obstacles" },
  { id: "mystery",       label: "Mystery/Whodunit",   icon: "🔍", description: "Puzzle-solving investigation" },
  { id: "custom",        label: "Custom Structure",   icon: "✨", description: "Describe your own screenplay format" },
];
...
// ── Character Arc Map ─────────────────────────────────────────────────────────
function CharacterArcMap({ characters }) {
  const radius = 120;
  const cx = 200, cy = 160;
  ...
  return (
    <svg width="100%" viewBox="0 0 400 320" style={{ fontFamily: C.mono }}>
      ...
      {edges.map(([a, b, type], i) => {
        const color = type === "romantic" ? C.crimson : type === "familial" ? C.emerald : type === "antagonistic" ? C.gold : C.dim;
        return (
          <line key={i}
            x1={nodes[a].x} y1={nodes[a].y} x2={nodes[b].x} y2={nodes[b].y}
            stroke={color} strokeWidth={2} strokeDasharray={type === "familial" ? "4 3" : "none"}
      ...
      {nodes.map((n, i) => {
        ...
        return (
          <g key={n.id} filter="url(#glow)">
            ...
            <text x={n.x} y={n.y - 6} textAnchor="middle" fill={genre.color} fontSize="12" fontWeight="700">{n.role || "?"}</text>
...
function ScriptArchitect({ onAccept, onCancel }) {
  ...
  const generatePlan = async () => {
    setLoading(true);
    setStep(2);
    setStatusMsg("AI is crafting the screenplay structure...");

    const sys = `You are a master screenwriter and story consultant. Design a complete screenplay structure.
...
Generate 3-6 key characters and 3-act structure with 4-8 scenes total. Be cinematic and compelling.`;

    const msg = `Archetype: ${archetype?.label}
Genre: ${genre}
Description: ${desc || "Craft a screenplay that captivates audiences"}
Design a complete cinematic structure.`;
    ...
    try {
      ...
      setStatusMsg("Screenplay inspiration failed.");
      setStep(1);
    } finally {
      setLoading(false);
    }
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 28 }}>
          ...
          {step === 0 && (
            <div>
              ...
              <p style={{ color: C.dim, fontSize: 13, marginBottom: 24, fontFamily: C.mono }}>What narrative structure shall your screenplay follow?</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={`Describe the central conflict, characters, or visual style that drives your screenplay...`}
                    rows={6}
                    style={{
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <p style={{ color: C.dim, fontSize: 13, lineHeight: 1.7, fontStyle: "italic" }}>
                  {plan.visualStyle} · {plan.pacing} pacing
                </p>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {plan.characters?.map((c, i) => (
                    <div key={i} style={{
                      ...
                      <span style={{ color: C.muted, fontSize: 12, flex: 1 }}>{c.arc}: {c.description}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {plan.structure?.map((act, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ color: C.dim, fontSize: 11, lineHeight: 1.4 }}>
                        {act.scenes} scenes: {act.keyElements?.join(" → ")}
                      </div>
                    </div>
                  ))}
                </div>
...
// ── Cinematic Dialogue (with screenplay) ───────────────────────────────────────
function CinematicDialogue({ scenes, characters, onApplyEdit }) {
  ...
  const send = async () => {
    ...
    const characterContext = characters.map(c => `${c.name} (${c.role}): ${c.arc}`).join("; ");

    const sys = `You are a screenplay consultant and film critic. Help develop and refine screenplays.
Script context: ${scriptContext || "No scenes yet"}
Characters: ${characterContext || "No characters yet"}

Answer the screenwriter's question about their screenplay. Suggest improvements, discuss character arcs, analyze visual storytelling, or explore cinematic techniques.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.panel, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
      <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontFamily: C.mono, fontSize: 11, color: C.emerald, letterSpacing: "0.1em" }}>CINEMATIC DIALOGUE</span>
      </div>

      <div style={{ flex: 1, overflow: "auto", padding: "16px 16px 8px" }}>
        {messages.length === 0 && (
          <div style={{ color: C.dim, fontSize: 12, fontFamily: C.mono, textAlign: "center", paddingTop: 40, lineHeight: 2 }}>
            Ask about your screenplay.<br/>
            "How does this scene play visually?" · "Character motivation?" · "Pacing issues?"
          </div>
        )}
        {messages.map((m, i) => (
          <div key={m.role === "user" ? i : i} style={{
            marginBottom: 14,
...
// ── Main ScreenplayForge ──────────────────────────────────────────────────────
export default function ScreenplayForge() {
  ...
  const generateSceneContent = useCallback(async (sceneId, planData, genre, allScenes, allCharacters) => {
    ...
    const characterSummary = allCharacters.map(c => `${c.name} (${c.role}): ${c.arc}`).join("; ");
    ...
    const sys = `You are a master screenwriter writing in ${genre} genre. Write compelling cinematic scenes.
Output ONLY the scene content in standard screenplay format.
Write visual, dialogue-driven scenes with proper formatting.`;

    const msg = `Screenplay: ${planData?.title || screenplayTitle}
Genre: ${genre}
...
Write the complete screenplay scene.`;
  ...
  // ── Accept screenplay plan and generate scenes ──────────────────────────────
  const handleAcceptPlan = useCallback(async (planData, genre) => {
    setShowArchitect(false);
    setScreenplayTitle(planData.title);
    ...
    planData.structure?.forEach(act => {
      for (let i = 0; i < act.scenes; i++) {
        newScenes.push({
          ...
          form: "screenplay", content: "", status: "queued", critique: null, notes: null,
        });
        sceneCount++;
      }
    });
    ...
    setGlobalStatus("Screenplay draft complete.");
  ...
  const critiqueScene = useCallback(async (scene) => {
    ...
    const sys = `You are a screenplay critic. Analyze the scene and return ONLY JSON:
{
  "score": number (1-10),
...
Be constructive and specific. Limit to 3 issues max.`;
  ...
  const expandScene = useCallback(async (scene) => {
    ...
    const sys = `You are a screenwriter expanding a scene. Add more visual detail, subtext, and cinematic elements.
Return ONLY the expanded scene content — no explanations.`;

    const msg = `Expand this scene with more cinematic depth:\n\n${scene.content}`;
  ...
  const addBlankScene = () => {
    ...
    const newScene = { id: uid(), title: name, act: 1, order: scenes.length + 1, form: "screenplay", content: "", status: "queued", critique: null, notes: null };
    setScenes(prev => [...prev, newScene]);
    setActiveSceneId(newScene.id);
  };

  const doneCount = scenes.filter(s => s.status === "done").length;
  const totalPages = Math.ceil(scenes.reduce((acc, s) => acc + (s.content?.split("\n").length || 0), 0) / 55); // ~55 lines per page
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontFamily: C.mono, color: C.gold, fontSize: 14, fontWeight: 700, letterSpacing: "0.12em" }}>🎬 SCREENPLAY FORGE</span>
        <span style={{ color: C.dim, fontSize: 12, fontFamily: C.mono }}>/{screenplayTitle}</span>
          ...
          {scenes.length > 0 && (
            <>
              <span style={{ fontFamily: C.mono, fontSize: 11, color: C.dim }}>
                {doneCount}/{scenes.length} scenes · ~{totalPages} pages
              </span>
          ...
          <div style={{ flex: 1, overflow: "auto", padding: "8px 8px" }}>
            {scenes.length === 0 && (
              <div style={{ padding: "30px 14px", color: C.dim, fontSize: 11, fontFamily: C.mono, textAlign: "center", lineHeight: 2 }}>
                No scenes.<br/>Start with 🎬 New Screenplay.
              </div>
            )}
            {scenes.map(s => {
        ...
        {sidePanel && (
          <div style={{
            ...
            {sidePanel === "characters" && (
              <>
                <div style={{ padding: "10px 14px", borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontFamily: C.mono, fontSize: 10, color: C.dim, letterSpacing: "0.1em", textTransform: "uppercase" }}>Character Arcs</span>
                </div>
                <div style={{ flex: 1, overflow: "auto" }}>
                  <CharacterArcMap characters={characters} />
                  {plan?.themes && (
            ...
            {sidePanel === "chat" && (
              <CinematicDialogue scenes={scenes} characters={characters} onApplyEdit={() => {}} />
            )}
            {sidePanel === "outline" && (
              <>
                <div style={{ padding: "10px 14px", borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontFamily: C.mono, fontSize: 10, color: C.dim, letterSpacing: "0.1em", textTransform: "uppercase" }}>Screenplay Outline</span>
                </div>
