// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// NOVEL FORGE — Interactive Novel Creation System
// Ported and adapted from CodeForge.jsx for literary narrative construction.
// Features: Story Architect, Character Builder, Plot Weaver, Scene Generator,
// Narrative Analyzer, Dialogue Coach, World Builder, Thematic Explorer.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ── Genre configs ─────────────────────────────────────────────────────────────
const GENRES = {
  fantasy:     { icon: "🏰", color: C.gold,     label: "Fantasy"     },
  scifi:       { icon: "🚀", color: C.sapphire, label: "Sci-Fi"      },
  romance:     { icon: "💕", color: C.crimson,  label: "Romance"     },
  mystery:     { icon: "🔍", color: C.emerald,  label: "Mystery"     },
  literary:    { icon: "📖", color: C.gold,     label: "Literary"    },
  ...
  drama:       { icon: "🎭", color: C.sapphire, label: "Drama"       },
};

const ARCHETYPES = [
  { id: "hero_journey",    label: "Hero's Journey",     icon: "⚔️", description: "Classic monomyth with call to adventure, trials, transformation" },
  { id: "coming_age",      label: "Coming of Age",       icon: "🌱", description: "Young protagonist discovers self through challenges and growth" },
  { id: "love_story",      label: "Love Story",          icon: "💑", description: "Romance-driven narrative with emotional arcs and relationships" },
  { id: "detective",       label: "Detective Mystery",   icon: "🕵️", description: "Investigation, clues, revelation of hidden truths" },
  { id: "dystopian",       label: "Dystopian Future",    icon: "🏙️", description: "Oppressive society, rebellion, social commentary" },
  { id: "epic_fantasy",    label: "Epic Fantasy",        icon: "🐉", description: "World-spanning adventure with magic, quests, ancient evils" },
  { id: "psychological",   label: "Psychological Drama", icon: "🧠", description: "Internal conflicts, mental states, character introspection" },
  { id: "historical_epic", label: "Historical Epic",     icon: "📜", description: "Sweeping historical events through personal stories" },
  { id: "supernatural",    label: "Supernatural Tale",   icon: "👻", description: "Ghosts, spirits, otherworldly forces in everyday life" },
  { id: "custom",          label: "Custom Structure",    icon: "✨", description: "Describe your own narrative architecture" },
];
...
// ── Character Relationship Graph (SVG) ─────────────────────────────────────────
function CharGraph({ characters }) {
  ...
  return (
    <svg width="100%" viewBox="0 0 400 320" style={{ fontFamily: C.mono }}>
      ...
      {nodes.map((n, i) => {
        const genre = GENRES[n.genre] || GENRES.literary;
        return (
...
function StoryArchitect({ onAccept, onCancel }) {
  ...
  const generatePlan = async () => {
    setLoading(true);
    setStep(2);
    setStatusMsg("AI is crafting your story architecture...");

    const sys = `You are a master novelist and story architect. Design a complete novel structure.
Return ONLY valid JSON: {
  ...
  "characters": [
    {
      "name": string,
      "role": string (protagonist|antagonist|mentor|love_interest|sidekick),
...
Generate 3-5 main characters and 3-act structure. Be creative and detailed.`;

    const msg = `Archetype: ${archetype?.label}
Genre: ${genre}
Description: ${desc || "Craft a compelling novel with this archetype"}
Design a complete novel structure.`;
    ...
    try {
      ...
      setStatusMsg("Story design failed.");
      setStep(1);
    } finally {
      setLoading(false);
    }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 28 }}>
          ...
          {step === 0 && (
            <div>
              <h2 style={{ color: C.text, fontSize: 18, fontWeight: 700, marginBottom: 6 }}>Choose a Narrative Archetype</h2>
              <p style={{ color: C.dim, fontSize: 13, marginBottom: 24, fontFamily: C.mono }}>What kind of story are you weaving?</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={`Describe your novel's core idea, world, conflict, or themes...`}
                    rows={6}
                    style={{
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <p style={{ color: C.dim, fontSize: 13, lineHeight: 1.7, fontStyle: "italic" }}>
                  {plan.wordCount?.toLocaleString()} words · {plan.pacing} pacing
                </p>
              </div>
...
// ── Narrative Chat (with story) ───────────────────────────────────────────────
function NarrativeChat({ scenes, characters, onApplyEdit }) {
  ...
  const send = async () => {
    ...
    const sys = `You are a literary consultant helping with novel development.
Story context: ${storyContext || "No scenes yet"}
Characters: ${charContext || "No characters yet"}

Answer the writer's question about their novel. Suggest improvements, plot ideas, character development, or writing techniques.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.panel, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
      <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontFamily: C.mono, fontSize: 11, color: C.emerald, letterSpacing: "0.1em" }}>NARRATIVE CHAT</span>
      </div>

      <div style={{ flex: 1, overflow: "auto", padding: "16px 16px 8px" }}>
        {messages.length === 0 && (
          <div style={{ color: C.dim, fontSize: 12, fontFamily: C.mono, textAlign: "center", paddingTop: 40, lineHeight: 2 }}>
            Ask about your novel.<br/>
            "How do I develop this character?" · "What's the conflict?" · "Plot hole here"
          </div>
        )}
        {messages.map((m, i) => (
          <div key={i} style={{
...
export default function NovelForge() {
  ...
  const generateSceneContent = useCallback(async (sceneId, planData, genre, allScenes, allChars) => {
    ...
    const sys = `You are a master novelist writing in ${genre} genre. Write compelling, character-driven prose.
Output ONLY the scene content — no chapter headings, no explanations.
Write vivid, engaging narrative with dialogue and description.`;

    const msg = `Novel: ${planData?.title || novelTitle}
Genre: ${genre}
...
Write the complete scene content.`;
  ...
  const handleAcceptPlan = useCallback(async (planData, genre) => {
    ...
    setGlobalStatus("Novel draft complete.");
  ...
  const critiqueScene = useCallback(async (scene) => {
    ...
    const sys = `You are a literary critic. Analyze the scene and return ONLY JSON:
{
  "score": number (1-10),
...
Be constructive and specific. Limit to 3 issues max.`;
  ...
  const expandScene = useCallback(async (scene) => {
    ...
    const sys = `You are a novelist expanding a scene. Add more detail, dialogue, and depth.
Return ONLY the expanded scene content — no explanations.`;

    const msg = `Expand this scene with more detail and depth:\n\n${scene.content}`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontFamily: C.mono, color: C.gold, fontSize: 14, fontWeight: 700, letterSpacing: "0.12em" }}>📖 NOVEL FORGE</span>
        <span style={{ color: C.dim, fontSize: 12, fontFamily: C.mono }}>/{novelTitle}</span>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 16 }}>
          {scenes.length > 0 && (
            <>
              ...
              {badge(plan?.genre || "fantasy", C.gold)}
            </>
          )}
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflow: "auto", padding: "8px 8px" }}>
            {scenes.length === 0 && (
              <div style={{ padding: "30px 14px", color: C.dim, fontSize: 11, fontFamily: C.mono, textAlign: "center", lineHeight: 2 }}>
                No scenes.<br/>Start with ✨ New Novel.
              </div>
            )}
            {scenes.map(s => {
        ...
        {sidePanel && (
          <div style={{
            ...
            {sidePanel === "chat" && (
              <NarrativeChat scenes={scenes} characters={characters} onApplyEdit={() => {}} />
            )}
            {sidePanel === "outline" && (
              <>
                <div style={{ padding: "10px 14px", borderBottom: `1px solid ${C.border}` }}>
