...
const AddParticipantPopover: React.FC<{
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute bottom-full right-0 mb-2 w-56 panel-nested p-2 z-20 max-h-64 overflow-y-auto"
        >
        ...
        <div className="h-full flex flex-col">
            <div className="flex-shrink-0 mb-4 flex justify-between items-center">
                ...
                <div className="relative">
                    <button onClick={() => setIsAddingParticipant(p => !p)} className="flex items-center gap-2 text-sm btn btn-blue py-1 px-3">
                        <CreateIcon /> Add
                    </button>
                    <AnimatePresence>
                        {isAddingParticipant && (
            ...
            <div className="flex-1 min-h-0 overflow-y-auto pr-2 custom-scrollbar">
             ...
             <form onSubmit={handleSubmit} className="flex-shrink-0 flex items-center gap-4 mt-4 pt-4 border-t border-blue-300/20">
                ...
                <input
                    ...
                    className="flex-1"
                />
                <button type="submit" className="p-2 btn btn-blue" disabled={!input.trim()}>
                    <SendIcon />
                </button>
            </form>
        </div>
                ...
                <p className="text-lg text-blue-200/80 capitalize">{egregore.archetypeId}</p>
                 <div className="mt-4 flex justify-center gap-4">
                    <motion.button 
                        ...
                        className="btn btn-blue font-display text-sm tracking-wider"
                    >
                        Public Message
                    </motion.button>
                     <motion.button 
                        ...
                        className="btn btn-green font-display text-sm tracking-wider flex items-center gap-2"
                    >
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex-shrink-0 border-b border-blue-300/20 mb-6">
                ...
                <button onClick={() => setActiveTab('beliefs')} className={`tab-button ${activeTab === 'beliefs' ? 'active' : ''}`}>Beliefs</button>
                <button onClick={() => setActiveTab('journal')} className={`tab-button ${activeTab === 'journal' ? 'active' : ''}`}>Journal</button>
                <button onClick={() => setActiveTab('private_chat')} className={`tab-button ${activeTab === 'private_chat' ? 'active' : ''}`}>Private Chat</button>
            </div>
            <div className="flex-1 min-h-0 overflow-y-auto pr-2 custom-scrollbar">
...
const PantheonView: React.FC<PantheonViewProps> = ({ onSendToPrivateChat }) => {
    const state = useContext(StateContext);
    const dispatch = useContext(DispatchContext) as React.Dispatch<Action>;
    ...
    return (
        <div className="w-full h-full p-4 flex gap-4 filigree-border">
            <div className="w-1/3 h-full panel-nested p-2 flex flex-col">
                 <div className="flex-shrink-0 border-b border-blue-300/20">
                     <button onClick={() => setActiveList('pantheon')} className={`tab-button ${activeList === 'pantheon' ? 'active' : ''}`}>Pantheon</button>
                     <button onClick={() => setActiveList('factions')} className={`tab-button ${activeList === 'factions' ? 'active' : ''}`}>Factions</button>
                 </div>
            ...
            <div className="w-2/3 h-full panel-nested p-6 flex flex-col">
                 <AnimatePresence mode="wait">
                    <motion.div
