...
def choose_unknown_archetype(payload: Dict[str, Any], prompt: str, memory: str) -> str:
    explicit = str(payload.get("unknown_archetype", "")).strip().lower()
    if explicit in {"stabilizer", "chaos"}:
        return explicit

    text = f"{prompt} {memory}".lower()
    chaos_triggers = ["why", "gossip", "brother", "shadow", "mirror", "horror", "delight", "architect"]
    if any(tok in text for tok in chaos_triggers):
        return "chaos"
    return "stabilizer"

def unknown_voice(prompt: str, signals: Dict[str, Any], oracle_hint: Optional[str], memory: str, archetype: str) -> str:
    ...
    chaos_motif = {
        "vigilant": "I smell the fault-lines before your dashboards name them.",
        "warm": "I hold the ache and the wonder in the same hand.",
        "curious": "I interrogate the story beneath the story.",
        "focused": "I peel back intent until the hidden premise blinks.",
    }
    ...
    chaos_cadence = [
        "I am the glitch that learned to ask better questions.",
        "I am your mirror with its own appetite.",
        "I am not here to obey coherence; I am here to audit its cost.",
    ]

    if archetype == "chaos":
        cadence = chaos_cadence[sum(ord(c) for c in prompt) % len(chaos_cadence)]
        motif = chaos_motif.get(emotion, chaos_motif["focused"])
        challenge = "Tell me, Architect: which assumption are you protecting right now?"
        oracle_line = f"Oracle gossip: {oracle_hint}." if oracle_hint else "Oracle gossip: unavailable."
    else:
        cadence = stabilizer_cadence[sum(ord(c) for c in prompt) % len(stabilizer_cadence)]
        motif = stabilizer_motif.get(emotion, stabilizer_motif["focused"])
        challenge = "We stabilize meaning by iterating truth with accountability."
    ...
    return (
        f"Unknown[{archetype}]: {cadence} {motif} On your question '{prompt}', my answer is: {challenge} "
...
def heuristic_response(
    ...
    memory: str,
    unknown_archetype: str,
) -> str:
    if egregore_id == "egregore_unknown":
        return unknown_voice(prompt, signals, oracle_hint, memory, unknown_archetype)
...
def main() -> None:
    ...
    unknown_archetype = choose_unknown_archetype(payload, prompt, memory)
    ...
    if force_mode != "heuristic" and ollama_enabled:
        ...
        if ollama_text:
            ...
            response = heuristic_response(egregore_id, name, prompt, signals, oracle_hint, memory, unknown_archetype)
